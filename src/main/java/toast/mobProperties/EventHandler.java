...
public class EventHandler {
	...
	/**
	 * Called by World.spawnEntityInWorld(). Entity entity = the entity joining
	 * the world. World world = the world the entity is joining.
	 *
	 * @param event
	...
	public void onEntityJoinWorld(EntityJoinWorldEvent event) {
		if (!event.world.isRemote && event.entity instanceof EntityLivingBase
			...
			if (EventHandler.DISABLED) {
				...
				boolean alive = event.entity.isEntityAlive();
				...
				if (initDat <= 0) {
					...
					if (mobProps != null) {
						if (initDat == 0) {
							...
							if (alive && !event.entity.isEntityAlive()) {
	...
	/**
	 * Called by EntityLiving.onDeath(). EntityLivingBase entityLiving = the
	 * entity dropping the items. DamageSource source = the source of the lethal
	 * damage. ArrayList<EntityItem> drops = the items being dropped. int
	 * lootingLevel = the attacker's looting level. boolean recentlyHit = if the
	 * entity was recently hit by another player. int specialDropValue =
	 * recentlyHit ? entityLiving.getRNG().nextInt(200) - lootingLevel : 0.
	 *
	 * @param event
	 *            The event being triggered.
	 */
	...
	public void onLivingDrops(LivingDropsEvent event) {
		if (!event.entityLiving.worldObj.isRemote) {
			...
			MobDropsInfo mobDrops = new MobDropsInfo(event.entityLiving, event.source, event.drops, event.lootingLevel,
	...
	/**
	 * Called by EntityPlayer.interactWith(). EntityPlayer entityPlayer = the
	 * player interacting. Entity target = the entity being right clicked.
	 *
	 * @param event
	 *            The event being triggered.
	 */
	...
	public void onEntityInteract(EntityInteractEvent event) {
		if (event.target != null && event.entityPlayer != null && !event.entityPlayer.worldObj.isRemote) {
			...
			if (held != null && held.getItem() == Items.stick && held.stackTagCompound != null
					&& held.stackTagCompound.getBoolean("MP|InfoWand")) {
				...
				if (generated != null) {
					event.entityPlayer
							.addChatMessage(
									new ChatComponentText("[Info Wand] Generated external nbt stats file \""
											+ generated.getName().substring(0,
													generated.getName().length() - FileHelper.FILE_EXT.length())
											+ "\" at:"));
					event.entityPlayer.addChatMessage(new ChatComponentText("    " + generated.getAbsolutePath()));
				} else {
					event.entityPlayer.addChatMessage(
							new ChatComponentText("[Info Wand] Failed to generate external nbt stats file!"));
				}
			}
		}
	}
	...
	/**
	 * Called by a number of methods. EntityPlayer entityPlayer = the player
	 * interacting. PlayerInteractEvent.Action action = the action being taken.
	 * int x = the x-coord of the block being interacted with, if any. int y =
	 * the y-coord of the block being interacted with, if any. int z = the
	 * z-coord of the block being interacted with, if any. int face = the side
	 * of the block being interacted with, if any. World world = the world being
	 * interacted with. Result useBlock = result for using the targeted block.
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		if (event.world != null && event.y >= 0 && event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK
				&& event.entityPlayer != null && !event.entityPlayer.worldObj.isRemote) {
			ItemStack held = event.entityPlayer.getHeldItem();
			if (held != null && held.getItem() == Items.stick && held.stackTagCompound != null
					&& held.stackTagCompound.getBoolean("MP|InfoWand")) {
				...
				if (tag.hasNoTags()) {
					event.entityPlayer
							.addChatMessage(new ChatComponentText("[Info Wand] Tile entity has no usable tags!"));
					return;
				}
